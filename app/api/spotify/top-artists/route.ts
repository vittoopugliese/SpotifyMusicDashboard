...
export async function GET(request: NextRequest) {
  return withAuth(request, async (token) => {
    ...
    const key = cacheKey(["top-artists", timeRange, limit, token.slice(-16)]);
    ...
    const data = await spotifyFetchWithUserToken<TopArtistsResponse>(`/me/top/artists?time_range=${timeRange}&limit=${limit}`, token);
