...
export default function FactsPage() {
  ...
  const generateRandomFact = async (): Promise<Fact | null> => {
    const categoryChoices = [
      () => generateYourMusicFact(),
      () => generateArtistQuirksFact(),
      () => generatePlaylistInsightsFact(),
      () => generateGlobalComparisonsFact(),
    ];

    const randomCategory = categoryChoices[Math.floor(Math.random() * categoryChoices.length)];
  ...
  const generateYourMusicFact = (): Fact => {
    ...
    if (decades.size > 0) {
      facts.push({
        icon: Calendar,
        category: "Your Music",
        fact: `You've listened to tracks from ${decades.size} different decades`,
        id: `decades-${Date.now()}`,
      });
    }
    ...
    if (years.size > 0) {
      facts.push({
        icon: TrendingUp,
        category: "Your Music",
        fact: `Your music collection spans from ${Math.min(...Array.from(years))} to ${Math.max(...Array.from(years))}`,
        id: `span-${Date.now()}`,
      });
    }

    if (userTracks.length > 0 && hours > 0) {
      facts.push({
        icon: Timer,
        category: "Your Music",
        fact: `If you played all your top tracks back-to-back, it would take ${hours} hours`,
    ...
    if (userTracks.length > 0) {
      facts.push({
        icon: Music,
        category: "Your Music",
        fact: `Your average track length is ${Math.round(avgDuration)} minutes`,
        id: `avg-duration-${Date.now()}`,
      });
    }
    ...
    if (artists.size > 0) {
      facts.push({
        icon: Users,
        category: "Your Music",
        fact: `You listen to music from ${artists.size} unique artists`,
        id: `artists-${Date.now()}`,
      });
    }
    ...
    if (genres.size > 0) {
      ...
      facts.push({
        icon: Music,
        category: "Your Music",
        fact: `${genrePercentage}% of your top artists make ${topGenre[0]} music`,
        id: `genre-${Date.now()}`,
      });
    }
    ...
    if (userTracks.length > 0) {
      ...
      if (popularTracks > 0) {
        ...
        facts.push({
          icon: Target,
          category: "Your Music",
          fact: `${popPercentage}% of your top tracks are mainstream hits (70+ popularity)`,
          id: `mainstream-${Date.now()}`,
        });
      }
    }
  ...
  const generateArtistQuirksFact = async (): Promise<Fact | null> => {
    ...
    if (artistTracks.length > 0) {
      facts.push({
        icon: Timer,
        category: "Artist Quirks",
        fact: `${randomArtist.name}'s average track is ${Math.round(avgDuration)} minutes long`,
        id: `duration-${Date.now()}`,
      });
    }

    // Genre analysis
    if (randomArtist.genres.length > 0) {
      facts.push({
        icon: Music,
        category: "Artist Quirks",
        fact: `${randomArtist.name} is known for ${randomArtist.genres.slice(0, 2).join(" and ")} music`,
        id: `genres-${Date.now()}`,
      });
    }
    ...
    if (artistTracks.length > 0) {
      ...
      facts.push({
        icon: TrendingUp,
        category: "Artist Quirks",
        fact: `${randomArtist.name}'s top tracks average a popularity of ${Math.round(avgPopularity)}/100`,
        id: `popularity-${Date.now()}`,
      });
      ...
      if (hits > 0) {
        ...
        facts.push({
          icon: Target,
          category: "Artist Quirks",
          fact: `${randomArtist.name} has ${hitPercentage}% of their top tracks as mainstream hits`,
          id: `hits-${Date.now()}`,
        });
      }
    }
    ...
    if (randomArtist.followers && randomArtist.followers.total > 1000000) {
      ...
      facts.push({
        icon: Users,
        category: "Artist Quirks",
        fact: `${randomArtist.name} has over ${millions} million Spotify followers`,
  ...
  const generatePlaylistInsightsFact = async (): Promise<Fact | null> => {
    ...
    if (playlistTracks.length > 0) {
      facts.push({
        icon: Music,
        category: "Playlist Insights",
        fact: `Your '${randomPlaylist.name}' playlist has ${playlistTracks.length} tracks`,
        id: `count-${Date.now()}`,
      });
    }
    ...
    facts.push({
      icon: Timer,
      category: "Playlist Insights",
      fact: `Your '${randomPlaylist.name}' playlist would take ${Math.round(hours * 10) / 10} hours to play completely`,
      id: `duration-${Date.now()}`,
    });
    ...
    if (uniqueArtists.size > 0) {
      ...
      facts.push({
        icon: Users,
        category: "Playlist Insights",
        fact: `Your '${randomPlaylist.name}' playlist features ${uniqueArtists.size} unique artists`,
        id: `artists-${Date.now()}`,
      });

      if (avgTracksPerArtist > 2) {
        facts.push({
          icon: Target,
          category: "Playlist Insights",
          fact: `Your '${randomPlaylist.name}' playlist has an average of ${Math.round(avgTracksPerArtist)} tracks per artist`,
          id: `avg-tracks-${Date.now()}`,
        });
      }
    }
    ...
    if (playlistTracks.length > 0) {
      ...
      facts.push({
        icon: TrendingUp,
        category: "Playlist Insights",
        fact: `Your '${randomPlaylist.name}' playlist has an average popularity of ${Math.round(avgPopularity)}/100`,
        id: `popularity-${Date.now()}`,
      });
      ...
      if (highPopTracks > playlistTracks.length * 0.5) {
        facts.push({
          icon: Target,
          category: "Playlist Insights",
          fact: `Over half of '${randomPlaylist.name}' consists of mainstream hits`,
          id: `mainstream-${Date.now()}`,
        });
      } else {
        facts.push({
          icon: Lightbulb,
          category: "Playlist Insights",
          fact: `'${randomPlaylist.name}' is a mix of mainstream hits and hidden gems`,
          id: `eclectic-${Date.now()}`,
        });
      }
    }
    ...
    const globalFacts = [
      {
        ...
        fact: "The most streamed song on Spotify has been played over 4 billion times",
        id: `streamed-${Date.now()}`,
      },
      {
        icon: Timer,
        category: "Global Comparisons",
        fact: "The longest track on Spotify is over 63 hours long (768 minutes)",
        id: `longest-${Date.now()}`,
      },
      {
        icon: Activity,
        category: "Global Comparisons",
        fact: "The average song tempo on Spotify is 120 BPM",
        id: `tempo-${Date.now()}`,
      },
      {
        icon: Target,
        category: "Global Comparisons",
        fact: "Most popular songs have a popularity score above 70",
        id: `popularity-${Date.now()}`,
      },
      {
        icon: Users,
        category: "Global Comparisons",
        fact: "The most followed artist on Spotify has over 100 million followers",
        id: `followers-${Date.now()}`,
      },
      {
        icon: Music,
        category: "Global Comparisons",
        fact: "There are over 100 million tracks available on Spotify",
        id: `tracks-${Date.now()}`,
      },
    ];
  ...
  return (
    <div className="p-6 space-y-6">
      <IconTitle icon={Radio} title="Facts" subtitle="Discover interesting facts about your music taste" />
