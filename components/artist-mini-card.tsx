...
export default function ArtistMiniCard({artist, simplified = false}: {artist: SpotifyArtist; simplified?: boolean}) {
  const content = (
    <Link href={`/artists/${artist.id}`} className={`${simplified ? '' : 'bg-muted/50 '}rounded-lg p-4 hover:bg-muted transition-colors flex flex-col items-center justify-between`} style={{userSelect: "none"}} >
